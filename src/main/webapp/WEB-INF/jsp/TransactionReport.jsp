...
<body>
	...
	<h2>Rabobank Customer Transaction Report</h2><br/>
		...
		<div id="records" ng-app="CSVParseApp" ng-controller="customerReportCtrl">
			...
			<h4>Valid Tansactions :</h4>
				<table align="center">
					...
					<c:forEach items="${validTransactions}" var="record">
						<tr>
							<td><c:out value="${record.getAccountNumber()}" /></td>
							<td><c:out value="${record.getReference()}" /></td>
							<td><c:out value="${record.getStartBalance()}" /></td>
							<td><c:out value="${record.getMutation()}" /></td>
							<td><c:out value="${record.getEndBalance()}" /></td>
							<td><c:out value="${record.getDescription()}" /></td>
						</tr>
					</c:forEach>
				</table>
			</c:if>
